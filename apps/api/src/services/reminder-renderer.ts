...
/**
 * Renderiza el texto de un recordatorio desde su plantilla (sin enviar).
 * Refleja la misma lógica del worker (apps/worker/src/services/template-renderer.ts)
 * para que el preview muestre exactamente lo que se enviaría por plantilla.
 */
export async function renderReminderMessage(reminder: any): Promise<string> {
  ...
  const template = await prisma.jwMessageTemplate.findUnique({ where: { type: templateType } });
