...
export default function Sidebar() {
  ...
  const navContent = (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 space-y-0.5">
        {nav.map((item) => (
          <a key={item.href} href={item.href} onClick={(e) => { e.preventDefault(); router.push(item.href); setOpen(false); }}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-[14px] transition-colors ${isActive(item.href) ? 'text-azure font-medium bg-azure/5' : 'text-ink/70 hover:bg-fog'}`}>
            {item.icon}
            {item.label}
          </a>
        ))}
      ...
      <div className="px-6 py-4 border-t border-silver-mist">
        <button onClick={() => { localStorage.removeItem('token'); router.push('/login'); }}
          className="text-[13px] text-graphite hover:text-ink transition-colors">Cerrar sesión</button>
      </div>
    </div>
  );
  ...
  return (
    <>
      {/* Mobile header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-snow border-b border-silver-mist px-4 h-12 flex items-center">
        <button onClick={() => setOpen(true)} className="p-1">
          <svg className="w-6 h-6 text-ink" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5M3.75 17.25h16.5"/></svg>
        </button>
        <span className="ml-3 font-semibold text-[15px] text-ink tracking-tight">JW Reminders</span>
      </div>
      {/* Mobile drawer */}
      {open && <div className="lg:hidden fixed inset-0 z-50"><div className="absolute inset-0 bg-black/20" onClick={() => setOpen(false)}/><div className="absolute left-0 top-0 bottom-0 w-[260px] bg-snow">{navContent}</div></div>}
      {/* Desktop sidebar */}
      <aside className="hidden lg:block w-[260px] h-screen sticky top-0 bg-snow border-r border-silver-mist">{navContent}</aside>
    </>
  );
}
